...
def draw_box_3d(image, corners, color=(0, 0, 255), distance=0.0):
    ''' Draw 3d bounding box in image
    ...
    for ind_f in range(3, -1, -1):
        ...
        if ind_f == 0:
            ...
            cv2.putText(image, dist+'m' ,(corners[f[1], 0]+10,corners[f[1], 1]+15),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
...
def show_rgb_image_with_boxes(img, labels, calib):
    ...
    for box_idx, label in enumerate(labels):
        ...
        if cls_id == 1:
            ...
            center = np.mean(corners_2d, axis=0)[:2].astype(np.int)
            cv2.circle(img, tuple(center), 3, (0,0,255), -1)
            if corners_2d is None:
                continue
        img = draw_box_3d(img, corners_2d, color=cnf.colors[int(cls_id)], distance=location[2])
